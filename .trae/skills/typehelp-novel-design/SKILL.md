...
description: "为 TypeHelp 文字推理游戏设计新剧本。提供真相优先方法论、三轴交叉矩阵、标签互引图、三性验证、10 项硬约束、10 个 Agent 协作提示词（8 设计 + 2 推理验证）。当用户要求设计 TypeHelp 剧本、启动新项目、或按既定方法论产出设计文档时调用。也适用于修复 tag 互引断裂、find/list 只搜已读文件、纯黑盒玩家无法发现新文件等线索断崖问题。"
---
...
> 完整方法论见 [references/spec.md](./references/spec.md)
...
| C4 | tag 字段承担互引 | 取代跳转 DAG。**实现要求**：Box passage 必须有 `$unlocked` 机制——读取文件时自动解锁其 tags 指向的文件，find/list 遍历 `$unlocked`（非 `$cache`），否则纯黑盒玩家无法发现新文件（详见 §8.14） |
...
> 完整 system prompt 见本 skill 目录下的 `prompts/`

| # | Agent | 文件 | 关键职责 |
|---|---|---|---|
| 00 | Director | [director.md](./prompts/director.md) | 立项、真相冻结、阶段门禁、冲突仲裁 |
| 01 | Truth Designer | [truth-designer.md](./prompts/truth-designer.md) | 仅写客观世界状态（不写谜题） |
| 02 | Inference Architect | [inference-architect.md](./prompts/inference-architect.md) | 三轴交叉矩阵 + 双证据 |
| 03 | File Designer | [file-designer.md](./prompts/file-designer.md) | 命名约定 + 3 段式文件内容 |
| 04 | Tag Graph Designer | [tag-graph-designer.md](./prompts/tag-graph-designer.md) | tags 互引图（取代 DAG） |
| 05 | Meta & Tutorial Designer | [meta-tutorial-designer.md](./prompts/meta-tutorial-designer.md) | 隐藏文件 + 渐进教程 + 结局 |
| 06 | Formal Verifier | [formal-verifier.md](./prompts/formal-verifier.md) | 三性检查（独立否决权） |
| 07 | Twine Implementer | [twine-implementer.md](./prompts/twine-implementer.md) | 设计 → TypeHelp HTML 1:1 翻译 |
| 08 | Playtester | [playtester.md](./prompts/playtester.md) | 黑盒试玩 + 卡点记录 |
| 09 | Inference Simulator | [inference-simulator.md](./prompts/inference-simulator.md) | 黑盒模拟玩家推理，产出 `inference_trace.json`（过程，需人在环触发） |
| 10 | Inference Grader | [inference-grader.md](./prompts/inference-grader.md) | 白盒判定 F 命中 + recall + 9 类失败归因，产出 `inference_grades.json` + `inference_report.md` |
...
> 完整协议见 [handoff-protocol.md](./prompts/handoff-protocol.md)
...
把 [startup-template.md](./prompts/startup-template.md) 中的 yaml 填好，发送给 Director Agent。
...
### 8.12 命令路由完整性陷阱（help 承诺 ≠ Box 实现）

**症状**：玩家按 `help` 输入 `act 1 抵岛` / `title 01 入职` / `hangman` / `find 江`，游戏返回"文件未找到"或"用法错误"——但 help 里明明列了这些命令。

**根因**：
- Twine Implementer 复制参考实现的 `Box` passage 时，**遗漏**了某些命令的 `<<elseif>>` 分支（`island-death` 曾缺失 `act`/`title`/`hangman` 三条）。
- 相关状态变量未在 `StoryInit` 初始化（如 `$titles` 未 `<<set $titles to {}>>`）。
- 隐藏文件 / meta 文件（如 `00-meta-warning`）只写在 `hidden_files.md` 里，**没有**在 `Box` passage 或对应文件 passage 中设计显式解锁路径。

**反模式**：
- ❌ 以为"参考实现有，复制过来就一定有"——`galley-villa` 的 `Box` 并不包含所有剧本需要的命令（如某些剧本需要 `act`，某些不需要）。
- ❌ 只 grep `Box` 里有没有命令名，不实际跑命令。
- ❌ `help` passage 和 `Box` passage 不同步：help 改了，Box 没改。
- ❌ 隐藏文件只列在设计文档，不设计触发条件。

**正确做法**：
- ✅ Twine Implementer 完成后，从 `help` passage 提取**完整命令清单**，在 `Box` passage 逐条核对 `<<elseif>>` 分支：
  ```
  help / list / back / save / load / inbox / name / title / note / find / act / hangman
  ```
  每缺一条 = 未完成。
- ✅ 检查 `StoryInit` 是否为每条命令所需变量赋初值：`$acts`（数组）、`$titles`（对象）、`$guessed_letters`（数组）、`$notes`（数组）、`$nicknames`（对象）等。
- ✅ 每条命令至少测 3 种输入：**无参数**、**合法参数**、**非法参数**。
- ✅ 对隐藏 / meta 文件，必须在 `Box` passage 或对应文件 passage 中写显式解锁逻辑：
  ```
  <<if ("23-04-PL-1" is $command) or ("24-04-PL-1" is $command)>>
    <<if not $cache.includes("00-meta-warning")>>
      <<run $cache.push("00-meta-warning")>>
    <</if>>
  <</if>>
  ```
- ✅ Formal Verifier 在"完备性"检查中增加"命令清单一致性"和"meta 文件解锁路径"两项；Playtester 在"命令探索型"测试中专测 help 列出的所有命令。

**自检问题**：
1. `help` 里列的每条命令，在 `Box` 里都有对应分支吗？
2. 每条命令依赖的变量都在 `StoryInit` 初始化了吗？
3. 每个隐藏 / meta 文件都有**玩家可触发**的解锁条件吗？
4. 非法参数返回的是统一风格的中文错误提示，还是中英混用 / 英文残留？

### 8.13 不要盲信参考实现（基线也可能有 bug）

**症状**：新剧本严格"对齐参考实现"，结果中文 sanitize 失败、stale state、英文错误消息等问题和 `galley-villa` 一模一样。

**根因**：
- `galley-villa` 是**参考实现**，不是**完美实现**。它的 `Box` passage 正则仍缺 CJK 范围，存在 stale-state 隐患，`list` 越界返回英文 "Invalid command."。
- `galley-villa/framework_diff.md` 曾错误声明 L10n 为"英文原文"，与文件内 41332 处中文字符矛盾——文档自指污染会让错误被不断复制。

**反模式**：
- ❌ "参考实现这么做，所以我也这么做"。
- ❌ 只对比命中数 / 行数，不看实际内容。
- ❌ 把 `framework_diff.md` 中的声明当事实，不验证实际 HTML。

**正确做法**：
- ✅ 把参考实现当**diff 基线**，不是**真理来源**。差异分 3 类处理：必须对齐 / 保留设计差异 / **修复原版 bug**。
- ✅ 对 `Box` passage、sanitize 正则、错误消息文案等**玩家高频接触点**，必须结合实际目标语言验证，不因参考实现未修就不修。
- ✅ 发现参考实现有 bug 时，在 `framework_diff.md` 的"修复原版 bug"章节显式记录，并同步更新本 skill 的教训（如本节）。

### 8.14 tag 解锁机制的设计-实现落差（C4 落地缺陷）

**症状**：Inference Simulator 黑盒试玩发现：act1（8 个文件）读完后，纯文本玩家无法发现 act2-act5 的 28 个文件。两次"通关"均依赖作弊（猜隐藏文件名 / 读 DOM data-tags 属性）。

**根因**（设计-实现落差）：
- **设计意图**（tag_graph.md / file_index.md / tutorial_design.md）：
  - C4："tag 字段承担互引，`tags="..."` 取代跳转 DAG"
  - file_index.md："你当前已**解锁**的文件可以通过 list 命令查看"
  - find 是"搜索相关文件"（暗示能发现未读文件）
- **实现现实**（Box passage 代码）：
  - `$cache` 只在玩家**输入正确文件名**时才 push
  - `find_results` 只遍历 `$cache` → 只搜已读文件
  - `list` 只遍历 `$cache` → 只列已读文件
...
**反模式**：
- ❌ 设计文档写"tag 互引"但不验证实现是否读取了 tag
- ❌ find/list 只遍历 `$cache`（已读文件），让玩家陷入"不知道文件名就无法发现文件"的死循环
- ❅ 静态分析（如 tag_graph.md 可达性检查）只验证 tag 链连通性，不验证 tag 是否被代码实际使用

**正确做法**（`$unlocked` 机制）：
- ✅ 新增 `$unlocked` 状态变量（已读 `$cache` 的超集，含 tag 指向但未读的文件）
- ✅ Box passage 文件名跳转分支中，读取文件后自动解锁其 tags 指向的文件：
  ```
  <<set _tags to tags($command)>>
  <<for _i to 0; _i lt _tags.length; _i++>>
    <<if not $unlocked.includes(_tags[_i]) and Story.has(_tags[_i]) and not _tags[_i].startsWith("00-")>>
      <<run $unlocked.push(_tags[_i])>>
    <</if>>
  <</for>>
  ```
- ✅ `find_results` 改为遍历 `$unlocked`（搜文件名+内容，返回文件名链接）
- ✅ `list` 改为遍历 `$unlocked`（已读/未读视觉区分，未读标记 `[未读]`）
- ✅ `00-` 前缀隐藏文件保持各自触发条件，不被 tag 自动解锁

**剧本层配合**：
- ✅ act1 末尾文件补充指向 act2 的搜索关键词提示（不剧透文件名，只给关键词）
- ✅ 如 `06-01-PL-1` 末尾："试试 find 江某 或 find 2017"
- ✅ NAS 等信息枢纽文件提示"输入 list 查看已解锁的文件"

**自检问题**（Twine Implementer 完成后必问）：
1. 我的 find/list 是遍历 `$cache` 还是 `$unlocked`？
2. 读完一个文件后，它的 tags 指向的文件是否自动加入 `$unlocked`？
3. 纯黑盒玩家（不猜文件名、不读 DOM）能否从 00-readme 推进到结局？——用 §8.15 的方法验证

### 8.15 纯黑盒推理验证方法（防作弊试玩）

**症状**：Inference Simulator 试玩"通关"了，但实际是靠 AI 上下文泄漏（猜隐藏文件名）或 DOM 检查（读 data-tags 属性）作弊——假通关掩盖了真实的线索断崖。

**根因**：
- AI Agent 有"快速归因"倾向——看到 list 是空的就猜文件名、看到 tag 不可见就读 DOM
- 没有强制约束 Agent 只看渲染文本、不接触源码/DOM 结构
- 验证方法本身不可靠 → 验证结果不可信

**反模式**：
- ❌ 让 Simulator Agent 直接读 HTML 源码"模拟玩家试玩"
- ❌ 允许 Agent 用 `page.evaluate()` 读取 DOM 属性（如 `data-tags`）
- ❌ 允许 Agent 猜文件名（如凭段号递增推导 `07-02-PL-1`）
- ❌ 只跑一次试玩就认为修复有效

**正确做法**（纯黑盒约束 + 多次独立验证）：
- ✅ **只通过 Playwright 操作文本框**：`page.locator('input#textbox-input')` 定位输入框，fill 命令后按回车
- ✅ **只提取渲染文本**：`page.evaluate(() => document.querySelector('#passages')?.innerText || '')`——只取玩家目视可见的文本
- ✅ **禁止读 DOM 结构**：不许读 data-tags / tw-passagedata / script 标签
- ✅ **禁止读源码**：不许用 Read 工具读 HTML 文件本身
- ✅ **禁止猜文件名**：只能通过 list 命令、find 命令、或文件文本里明确提到的文件名
- ✅ **多次独立验证**：至少跑 2 次采用不同探索策略的试玩（如顺序驱动 vs find 驱动），都达成 true_ending 才算修复稳定
- ✅ **产出 inference_trace.json**：记录每步的命令、响应摘要、推理、证据，便于 Grader 白盒判定
- ✅ **honestAssessment 字段**：要求 Agent 对是否作弊做诚实自评

**验证通过标准**：
- 两次独立试玩都达成 `true_ending`
- 无连续 3 步无新信息的卡死点
- honestAssessment 确认无作弊
- 每条推理引用具体文件名 + 原文片段（证据锚定）

**自检问题**：
1. 我的 Simulator Agent 是否只看渲染文本，不读 DOM/源码？
2. 是否跑了至少 2 次不同策略的独立试玩？
3. Agent 的 honestAssessment 是否确认无作弊？
...
位于本 skill 目录下的 `references/`：
- [spec.md](./references/spec.md) — 完整方法论（含 9 项硬约束推导）
- [tasks.md](./references/tasks.md) — 任务清单
- [checklist.md](./references/checklist.md) — 验证清单

### 9.2 Agent 提示词

位于本 skill 目录下的 `prompts/`：
- README.md
- startup-template.md
- handoff-protocol.md
- director.md
...
- playtester.md
- inference-simulator.md
- inference-grader.md

### 9.3 Worked Example

`example-blackbox/` — 「黑匣子 / Black Box」太空船推理游戏，含完整设计文档（.md + .json）。HTML 源码因体积较大（707KB embedded / 44KB source），保留在 `.trae/specs/typehelp-novel-design/example-blackbox/`，不复制到 skill 目录。
...
| 纯黑盒玩家读完整 act1 后无法发现 act2 文件 | C4 tag 互引只在设计文档中描述，实现中 find/list 只遍历 `$cache`（已读文件），tag 字段从未被代码读取——玩家陷入"不知道文件名就无法发现文件"的死循环（详见 §8.14） | 实现 `$unlocked` 机制：读取文件时自动解锁其 tags 指向的文件加入 `$unlocked`，find/list 改为遍历 `$unlocked`；act1 末尾文件补充搜索关键词提示（如"试试 find 江某"） |
| Inference Simulator"通关"但实际作弊 | Agent 通过 AI 上下文泄漏猜隐藏文件名，或用 `page.evaluate` 读 DOM `data-tags` 属性获取文件互引——假通关掩盖真实线索断崖（详见 §8.15） | 纯黑盒约束：只操作文本框 + 只提取 `#passages` innerText + 禁止读 DOM/源码/猜文件名；至少跑 2 次不同策略的独立试玩都达成 true_ending 才算修复稳定 |

---

## 11. 一句话心法

> **TypeHelp 剧本设计 = 为一个"终端"设计 N 个有"自描述文件名 + 互引标签 + 视角差异 + 教程解锁"四元组的文件，并保证这 N 个文件在 $cache 累加到阈值时，拼出唯一真相。tag 互引不仅写在设计文档里，还必须被 `$unlocked` 机制实际执行——纯黑盒玩家不猜文件名也能从 00-readme 推进到结局。**
