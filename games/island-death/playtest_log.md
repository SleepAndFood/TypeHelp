...
## 9. 玩家实测反馈与体验修复（2026-06-29）

### 反馈来源

作者/玩家亲自试玩后提出的 6 条体验问题。

### 问题与修复

| # | 玩家反馈 | 修复方式 | 验证 |
|---|---|---|---|
| E-1 | 开头打字机没打完全部内容就切到 Intro | `Background` passage 的 `<<timed>>` 从 5s 延长到 11s，覆盖全部 6 行 type 动画 | 渲染测试 PASS |
| E-2 | 第一次输 help 还在原地，需要输两次 | 移除 `Box` 中 `$first_help` 的 Intro 重显逻辑；help 命令始终直接显示 `help` passage；清理未使用的 `$first_help` / `$first_list` | 渲染测试 PASS |
| E-3 | Intro 直接显示"2017—2024 双时间线"，建议隐藏 | 从 `Intro` 中移除双时间线字样，让玩家通过文件自行发现时间跨度 | 渲染测试 PASS |
| E-4 | "所有证据以文件形式存储"缺少不给出全部文件的理由 | `00-readme` 改为：NAS 硬盘曾被多次格式化，目录索引损坏，只能逐步恢复文件 | 渲染测试 PASS |
| E-5 | 文件命名规则不应直接给出，应让玩家自己总结 | `00-readme` 移除"段号-幕号-在场者缩写-在场人数"的直接说明，改为"观察多个文件的命名方式，你也许能总结出 NAS 的归档规律" | 渲染测试 PASS |
| E-6 | `00-invitation` 没有下一篇信息 | 在 `00-invitation` 末尾增加便签提示："邀请函背面贴着一张快艇班次便签，上面写着'01-01-PL-1'" | 渲染测试 PASS |
