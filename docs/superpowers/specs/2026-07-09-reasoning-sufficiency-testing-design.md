...
| 层 | 工具 | 触发 | 成本 | 入口 |
|---|---|---|---|---|
| **L6a 静态构图** | Node.js 脚本 | 标注完成后人工触发 | 0（秒级） | `npm run reasoning:build-graph` |
| **L6b 静态校验** | Vitest（纯 JS 图分析） | 每次 push / PR | 0（毫秒级） | `npm test`（自动含） |
...
**关键原则**：
- L6a 产出 `static-reasoning.json` 入仓（设计期产物）
- L6b 消费入仓的 `static-reasoning.json`，CI 不产出图，只重算可达性 + 断言
- L6 Agent 推理模拟**不进 CI**（需要人在 IDE 启动 Agent），但 L6 的**产出**（`inference_report.md` + `inference_trace.json`）入仓后**被 CI 校验存在性 + 格式**
...
| 解锁图 | File → File | HTML passage body 的 `<<run $cache.push("filename")>>`（直接可解析：调用方 → 参数文件名） | 读完 A 后文件 B 被推入 cache，变为可访问 |
| 揭露图 | File → Fact | `file_index.md` 的 `exposes: [F1, F2]` 标注 | A 文件正文显式揭露 F 事实 |

> **注意**：`<<set $seen_xxx>>` 标志机制需要交叉引用（设标志的文件 ≠ 检查标志的文件），无法从单文件 body 直接解析。PoC 阶段仅用 `$cache.push()` 构建解锁图。如果发现解锁图覆盖不足（大量隐藏文件无入边），再扩展到 `$seen_xxx` 交叉引用（在 Box passage 的命令路由逻辑中搜索 `<<if $seen_xxx>>` 条件）。

**可达性定义**：F 可达 = tag 图可达 ∪ 解锁图可达（两张图都不可达才判定为不可达）。

**两种异常节点定义**（区分 unreachable vs dead-end）：

| 类型 | 定义 | 含义 |
|---|---|---|
| 不可达文件 | 非结局文件在 tag 图 ∪ 解锁图中**无入边**（没人指向它） | 玩家无法到达此文件 |
| 死胡同文件 | 非结局文件在 tag 图 ∪ 解锁图中**无出边**（它不指向任何人） | 玩家到达后无处可去 |
...
```
对每个公开文件 F:
  1. tag 图入边 ∪ 解锁图入边 = 空 AND F 不是结局文件 AND F 不是起始文件 → 不可达文件
  2. 多个不可达文件形成连通分量 → 信息孤岛
```

**算法 3 — 死胡同检测**：
```
对每个公开文件 F:
  1. tag 图出边 ∪ 解锁图出边 = 空 AND F 不是结局文件 → 死胡同文件
  2. 死胡同文件 ≠ 不可达文件（前者可达但无处可去，后者根本到不了）
```

**算法 4 — 双证据可达性检查**：
```
对每个 F:
  1. exposes_in[F] 中可达文件数 < 2 → 标记 "可达证据不足" 警告
  2. exposes_in[F] 中可达文件全来自同一视角（同一人物 ID / 同一地点码）→ 标记 "视角单一" 警告
```
...
| L6b（CI，每次 push） | 每次 push / PR | 校验 `static-reasoning.json` 格式 + 基于入仓图重算 F 可达性 + 不可达文件 + 死胡同 + 双证据（不重建图） | 报告不入仓 |
...
- `unreachableFiles.length > 0` → CI fail（非结局/非起始文件无人指向）
- `deadEndFiles` → 警告（不 fail，提示 Director 审查 — 可能是合理的终点文件）
...
**baseline 的 Grader 判定规则不同**：baseline 没有 `evidence_collected`（因为没读文件），Grader 直接对 baseline 的 `finalInference` 做语义匹配判定 F 命中——**不适用** §5.4 的证据锚定约束（证据锚定只约束真实 Simulator，不约束 baseline）。这样 baseline 可以获得非零 recall（凭类型套话猜对部分 F）。
...
Grader 是 LLM Agent，用**语义匹配**判定 F 命中（非纯子串匹配——中文语序变化如"赵某推江某" vs "江某被赵某推"无法靠子串匹配覆盖）。

对每个 F：
1. 从 truth.md 提取 `verifiable_claims`（作为判定的锚点，缩小 LLM 主观范围）
2. 扫描 Simulator 的 `evidence_collected` 中的 `claim` 字段
3. Grader 逐条判定每个 verifiable_claim 是否被 Simulator 的任一 claim **语义覆盖**（同义、近义、不同语序表达均算覆盖）
4. 语义覆盖的 verifiable_claims 比例 ≥ 60% → 判定 F 命中
5. 记录命中的 claim 列表 + 对应文件

**verifiable_claims 的作用**：不是做纯子串匹配，而是给 Grader 的 LLM 判定**提供锚点**——Grader 必须对每个 claim 逐一判定"Simulator 的推理是否覆盖了这个 claim"，而非笼统判定"Simulator 是否推出了 F"。这把 LLM 的主观判断约束在结构化的逐项检查中，降低幻觉风险。
...
| 死胡同 | Simulator 连续 3 步无新文件，且 Grader 分析 trace 后排除其他类型（非命名问题、非命令未解锁、非信息不足） | Tag Graph Designer |
...
agent_profile.md 分为两部分：**Simulator 可见部分**（§0-§2, §4-§5）和**仅操作者可见部分**（§3 禁止透露项）。launcher 脚本组装 Simulator 提示词时**剥离 §3**，仅将 §0-§2, §4-§5 注入 Simulator prompt。
...
## 1. 玩家角色（Simulator 可见，meta 元素时必须填写）
- 玩家身份：<如"第 4 任管家">
...
**L6a / L6b 分离**：
- L6a（`scripts/build-reasoning-graph.js`）：人工触发，解析 HTML `tags=""` 属性 + `<<run $cache.push("filename")>>` + `file_index.md` 的 `exposes` 标注，构建图，产出 `test/reasoning/_reports/<game>.static-reasoning.json` 入仓
...
- 4 个文件：`00-readme`（起始）→ `01-ST-1` → `02-BR-1`（结局），`03-OR-1`（无入边——不可达）
- 2 个 F：F1 被 `01-ST-1` 揭露，F2 被 `02-BR-1` 揭露
- `03-OR-1` 无任何 tag 指向它（无入边），非起始非结局 → 不可达文件
- 预期：`unreachableFacts = []`，`unreachableFiles = ["03-OR-1"]`，`deadEndFiles = []`（02-BR-1 是结局文件不算死胡同）
...
- **剥离 §3 + §5**（仅操作者可见部分），将 §0-§2, §4 注入 Simulator 提示词
...
3. 从 island-death HTML 提取 `tags=""` 属性 + `<<run $cache.push("filename")>>` 解锁逻辑
...
| Grader 判定不准 | 中 | recall 不可信 | verifiable_claims 提供结构化锚点，Grader 逐项语义判定而非笼统判定；证据锚定约束迫使 Simulator 的 claim 有文件出处可追溯 |
